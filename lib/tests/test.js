...
const playlistsCtx = new MyORMContext(pool, "Playlist");

/** @type {MyORMContext<import("./bb.js").User>} */
const bb = new MyORMContext({ host: "baby-beluga-db.mysql.database.azure.com", user: "babybelugaadmin", password: "bbManagement1@3$", database: "baby_beluga" }, "User");
...
async function customTest() {
    await bb.hasOne(m => m.Credential
        .withPrimary("Id")
        .withForeign("UserId"));

    await bb.hasOne(m => m.ClientInfo
        .withPrimary("Id")
        .withForeign("UserId"));

    await bb.hasMany(m => m.UserRoles
        .from("UserRole")
        .withPrimary("Id")
        .withForeign("UserId")
    ...
    await bb.hasMany(m => m.UserRoomLayouts
        .from("UserRoomLayout")
        .withPrimary("Id")
        .withForeign("UserId")
        .andThatHasOne(m => m.RoomLayout
            .withPrimary("RoomLayoutId")
            .withForeign("Id")
            .andThatHasOne(m => m.Room
                .withPrimary("RoomId")
                .withForeign("Id"))));

    await trackCtx.hasOne(m => m.Album
        .withPrimary("AlbumId")
        .withForeign("AlbumId"));
    await trackCtx.hasOne(m => m.Artist
        .withPrimary("Composer")
        .withForeign("Name"));
    await trackCtx.hasOne(m => m.Genre
        .withPrimary("GenreId")
        .withForeign("GenreId"));
    await trackCtx.hasOne(m => m.MediaType
        .withPrimary("MediaTypeId")
        .withForeign("MediaTypeId"));
    await trackCtx.hasOne(m => m.PlaylistTrack
        .withPrimary("TrackId")
        .withForeign("TrackId")
    ...
    await playlistsCtx.hasMany(m => m.PlaylistTracks.from("PlaylistTrack")
        .withPrimary("PlaylistId")
        .withForeign("PlaylistId")
            .andThatHasOne(m => m.Track
                ...
                .andThatHasOne(m => m.MediaType
                    .withPrimary("MediaTypeId")
                    .withForeign("MediaTypeId"))));
    console.log({schema: playlistsCtx._schema, relationships: playlistsCtx._relationships});
    //@ts-ignore
    console.log(playlistsCtx._schema);
    console.log(bb._relationships);
    const users = await bb.include(m => m.Credential)
        .include(m => m.ClientInfo)
        .include(m => m.UserRoles
            .thenInclude(m => m.Role))
        .include(m => m.UserRoomLayouts
            .thenInclude(m => m.RoomLayout
                .thenInclude(m => m.Room)))
        .take(2)
        .where(m => m.FirstName
            .eq("Travis"))
        .select();
    console.log(`Users`, { totalNum: users.length, user: users[0], userRoles: users[0].UserRoles, role: users[0].UserRoles[0].Role });

    console.log(trackCtx._schema, trackCtx._relationships);
    const tracks = await trackCtx.include(m => m.PlaylistTrack
        .thenInclude(m => m.Playlist))
        .take(2)
        .where(m => m.Composer.equals("AC/DC"))
        .select();
    console.log(`Tracks`, { totalNum: tracks.length, track: tracks[0], pTrack: tracks[0].PlaylistTrack, playlist: tracks[0].PlaylistTrack?.Playlist });
    
    const playlists = await playlistsCtx
        .include(m => m.PlaylistTracks
            .thenInclude(m => m.Track))
        .where(m => m.Name.like("Classical%"))
        .select();
    console.log(`Playlists`, { totalNum: playlists.length, playlist: playlists[1], pTracks: playlists[1].PlaylistTracks, track: playlists[1].PlaylistTracks[0].Track });
    process.exit();
}
