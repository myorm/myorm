    ...
    const playlistsWithTracks = await playlistsCtx
        ...
        .alias(m => ({
            ...
            tracks: m.PlaylistTracks.map(pt => ({
                id: pt.Track.TrackId,
                name: pt.Track.Name,
                price: pt.Track.UnitPrice,
                genre: {
                    id: pt.Track.Genre.GenreId,
                    name: pt.Track.Genre.Name
                }
            }))
        }))
        .take(5)
        ...
        .sortBy(m => [
            m.PlaylistId.asc(),
            m.PlaylistTracks.Track.Genre.Name,
            m.PlaylistTracks.Track.Bytes.desc()
        ])
        .select();
    console.log(playlistsWithTracks.length, playlistsWithTracks[0].tracks);
    process.exit();

    // GROUP BY TODO: make it so you can do something like: (m, aggr) => [m.Name, aggr.max(m.PlaylistTracks.Track.Bytes)]
}
