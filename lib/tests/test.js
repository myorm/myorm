...
async function customTest() {
    ...
    await playlistsCtx.hasMany(m => m.PlaylistTracks
    ...
    const playlistsWithTracks = await playlistsCtx
        .include(m => m.PlaylistTracks
            .thenInclude(m => m.Track))
        .take(5)
        .where(m => m.Name.equals("Music")
            .and(m => m.PlaylistTracks.Track.Bytes.greaterThan(50000)))
        .sortBy(m => [
            m.PlaylistTracks.Track.GenreId.desc(), 
            m.PlaylistTracks.Track.Milliseconds.asc()
        ])
        .select();
    console.log(playlistsWithTracks[0].PlaylistTracks);
    // const classicalPlaylists = await playlistsWithTracks.where(m => m.Name.contains("Classical")).select();
    // const musicPlaylists = await playlistsWithTracks.where(m => m.Name.contains("Music")).select();
    // console.log(`Music Playlists`, { 
    //     totalNum: classicalPlaylists.length, 
    //     playlist: classicalPlaylists[1], 
    //     pTracks: classicalPlaylists[1].PlaylistTracks, 
    //     track: classicalPlaylists[1].PlaylistTracks[0].Track 
    // });
    // console.log(`Classical Playlists`, { 
    //     totalNum: musicPlaylists.length, 
    //     playlist: musicPlaylists[1], 
    //     pTracks: musicPlaylists[1].PlaylistTracks, 
    //     track: musicPlaylists[1].PlaylistTracks[0].Track 
    // });
    process.exit();
}
