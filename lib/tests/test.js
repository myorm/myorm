...
async function customTest() {
    /** @type {MyORMContext<import("../../.github/chinook-setup/chinook-types.js").Track>} */
    const trkCtx = new MyORMContext(pool, "Track");
    await trkCtx
        .hasOne(m => m.PlaylistTrack.with("TrackId").to("TrackId")
            .andThatHasOne(m => m.Playlist.with("PlaylistId").to("PlaylistId")));

    /** @type {MyORMContext<import("../../.github/chinook-setup/chinook-types.js").Playlist>} */
    const playlistCtx = new MyORMContext(pool, "Playlist");
    playlistCtx.onFail(onFail);
    await playlistCtx
        .hasMany(m => m.PlaylistTracks.from("PlaylistTrack").with("PlaylistId").to("PlaylistId")
            .andThatHasOne(m => m.Track.with("TrackId").to("TrackId")
                .andThatHasOne(m => m.Album.with("AlbumId").to("AlbumId"))
                .andThatHasOne(m => m.Artist.with("Composer").to("Name"))
                .andThatHasOne(m => m.Genre.with("GenreId").to("GenreId"))
                .andThatHasOne(m => m.MediaType.with("MediaTypeId").to("MediaTypeId"))));
    const ps = await playlistCtx
        ...
        .where(m => m.Name.equals("Classical"))
        .take(1)
        .select();
    console.log(ps);
        // .alias(m => ({
        ...
        //     tracks: [{
        //         track: {
        //             id: m.PlaylistTracks.Track.TrackId,
        //             name: m.PlaylistTracks.Track.Name,
        //             genre: {
        //                 id: m.PlaylistTracks.Track.GenreId
        //             }
        //         }
        //     }]
        // }));

    // console.log(playlistCtx._relationships);
    // playlistCtx.onSuccess(onSuccess);
