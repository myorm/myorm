...
// import { adapter } from '../../../adapters/mysql/mysql.js';

// const pool = MyORMContext.createPool({ host: "192.168.1.9", port: 10500, database: "chinook", user: "root", password: "root" });
const pool = MyORMContext.createPool({ database: "chinook", host: "192.168.1.9", user: "root", password: "root", port: 10500 });
...
async function doCustom() {
    const playlists = await playlistsCtx
        .where(m => m.Name.not().equals("Music"))
        .select();
    process.exit();
}