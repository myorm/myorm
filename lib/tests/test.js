...
const pool = MyORMContext.createPool({ host: "localhost", port: 3306, database: "chinook", user: "root", password: "root" });
...
async function customTest() {
    const tracksAndAlbumView = trackCtx
        .include(m => [m.Album, m.Artist])
        .map(m => ({
            id: m.TrackId,
            name: m.Name,
            composer: m.Composer,
    ...
    //         genreId: m.GenreId
    //     }));
    var ts = await tracksAndAlbumView.where(m => m.Composer.eq("AC/DC")).select();
    console.log(ts.map(t => ({c: t.composer, n: t.name })));
    var ts = await tracksAndAlbumView.where(m => m.Composer.eq("Apocalyptica")).select();
    console.log(ts.map(t => ({c: t.composer, n: t.name })));
}
