...
export class WhereBuilder {
    ...
    constructor(column, table, relationships, chain="WHERE") {
        // if chain is a negation, then the depth should start 1 in, that way everything is wrapped with a parentheses.
        // @ts-ignore
        this._current = { depth: chain.endsWith("NOT") ? 1 : 0, chain, property: `\`${table}\`.\`${column}\`` }
        this._table = table;
        this._conditions = [];
...
function toStringRecursive(conditions, table, x=0) {
        ...
        for (const conds of conditions) {
            if (!Array.isArray(conds)) {
                if(String(conds.property).includes(table)) {
                    s += `${Array.from(Array(x + 1 + (conds.chain != "WHERE" ? 1 : 0)).keys()).map(_ => '\t').join('')}${conds.chain} ${String(conds.property)} ${conds.operator} ${Array.isArray(conds.value) ? `(${conds.value.map(_ => '?').join(',')})` : '?'}`;
                }
            } else {
                if (conds.length > 1) {
                    ...
                    if (cond.property.includes(table)) {
                        // @ts-ignore ignoring because recursive types are strange.
                        s += `${Array.from(Array(x + 1 + (cond.chain != "WHERE" ? 1 : 0)).keys()).map(_ => '\t').join('')}${cond.chain} (${cond.property} ${cond.operator} ${Array.isArray(cond.value) ? `(${cond.value.map(_ => '?').join(',')})` : '?'}${toStringRecursive(conds, table, x+1)})`;
                    }
                } else {
                    const cond = conds.shift();
                    // @ts-ignore ignoring because recursive types are strange.
                    if (cond.property.includes(table)) {
                        // @ts-ignore ignoring because recursive types are strange.
                        s += `${Array.from(Array(x + 1 + (cond.chain != "WHERE" ? 1 : 0)).keys()).map(_ => '\t').join('')}${cond.chain} ${cond.property} ${cond.operator} ${Array.isArray(cond.value) ? `(${cond.value.map(_ => '?').join(',')})` : '?'}`;
                    }
                }
            }
        }
    ...
    if(String(conditions.property).includes(table)) {
        return `${Array.from(Array(x + 1 + (conditions.chain != "WHERE" ? 1 : 0)).keys()).map(_ => '\t').join('') }${conditions.chain} ${String(conditions.property)} ${conditions.operator} ${Array.isArray(conditions.value) ? `(${conditions.value.map(_ => '?').join(',')})` : '?'}`;
    }
    return "";
}
