...
export class MyORMContext {
     ...
     * @type {{[K in keyof TTableModel]: DescribedSchema}} */ #schema;
    ...
    constructor(adapter, table, tableOptions={}) {
        ...
        this.#promise = this.#describe(table).then(async schema => {
            ...
            this.#schema = /** @type {{[K in keyof TTableModel]: DescribedSchema}} */ (Object.fromEntries(Object.entries(schema).map(([k,v]) => [v.field, v])));
            const scope = { MyORMAdapterError: (/** @type {string} */ msg) => new MyORMAdapterError(msg), Where };
    ...
    async insert(records) {
        ...
        if(Object.values(this.#schema).filter(c => c.isVirtual).length > 0) {
            ...
            const where = Where(columns[0], this.#table, this.#relationships, this.#schema);
