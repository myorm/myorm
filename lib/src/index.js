...
export class MyORMContext {
    ...
    async insert(records) {
        await this._promise;
        if (this._constraints.length > 0) throw new MyORMConstraintError(`The table, ${this._table} has constraints on it that prevent this function from being usable.`);
    ...
    */
     ...
     */
    #getOrderOfInsertion(processedConstraints=new Set(), table=this._table, relationships=this._relationships, constraints=this._constraints) {
        // let map = [];
        let order = [];
