...
export class MyORMContext {
     ...
     * @param {Map<string, boolean>} order 
     */
    async #cascadeInsert(records, order, relationships = this.#relationships, schema=this.#schema, constraints=this.#constraints) {
        console.log(order);
        const recs = records.map(r => r[order]);
        console.log(recs);
    }
    ...
    async insert(records) {
        ...
        if(this.#options.cascadeInserts && this.#cascade) {
            ...
            await this.#cascadeInsert(records, order);
        } else {
    ...
    async update(records) {
        ...
        const where = Where(pKey, this.#table, this.#relationships, this.#schema);
    ...
    */
    ...
    identify(callback) {
         ...
         *   Y: number;
         * };
         * const foo = new MyORMContext<Foo>(adapter, "Foo");
         * ctx.default((m, ref) => {
         *      m.Id.with(v4()); // auto assigning id
         *      m.Bar.Id.with(v4()); // auto assigning id
         *      m.X.with(`Hello world!`); // if undefined, "hello world!".
         * });
         * 
         * const recs = ctx.insert({
         *   Y: 4,
         *   Bar: {
         *     Y: 3
         *   }
     ...
     */
    #getPrimaryKey(table=null, relationships=this.#relationships) {
        let key = undefined;
