 ...
 * @template {Types.AbstractModel} TTableModel Model that represents the Table this Context represents.
...
export class MyORMContext {
     ...
     * @protected @type {Partial<{[K in keyof Types.OnlyAbstractModels<TTableModel>]: { included: boolean, name: string, primaryKey: keyof TTableModel, foreignKey: string, type: "1:1"|"1:n" }}>} 
     */ 
    _relationships = {};
     ...
     * @protected @type {Types.SortByKeyConfig[]} 
     */ 
    _sortByKeys = [];
     ...
     * @protected @type {Types.ViewConfig<TTableModel>} 
     */ 
    _view = {
        where: null,
        limit: 0,
     ...
     * @param {Types.MyORMAdapter<TTableModel>} adapter `mysql2` config options to create a Pool object with or an existing Pool.
     ...
     */
    constructor(adapter, realTableName, options = {}, schema = undefined) {
     ...
     * @param {Types.HasManyCallback<TTableModel>} relationshipCallback Used to configure the keys for the informal foreign relationship.
     * @example
     * ```js
     ...
     * 
     * You can further configure relationships from that related table by chaining `.andThatHasOne()` or `.andThatHasMany()` onto the respective properties.
     * @param {Types.HasOneCallback<TTableModel>} relationshipCallback Used to configure the keys for the informal foreign relationship.
     * @example
     * ```js
     * const pool = MyORMContext.createPool({ host: "localhost", database: "test", user: "root", password: "root" });
     * const userContext = new MyORMContext(pool, "User");
     ...
     * @template {Types.OnlyAbstractModelTypes<TTableModel>} [TAugmentedType=Types.OnlyAbstractModelTypes<TTableModel>] TTableModel augmented so it only displays Abstract Model types. This will be inferred.
     * @param {Types.ThenIncludeCallbackConfig<TTableModel>} modelCallback 
    ...
    include(modelCallback) {
        ...
        const ctx = new MyORMContext(this._adapter, this._realTableName, this._options, this._schema);
     ...
     * @param {(m: Types.ChainObject<TTableModel>) => void} whereCallback Builder function to help build a WHERE clause.
        ...
        this._promises = [Promise.all(this._promises).then(() => {
            const newProxy = (table) => new Proxy(/** @type {any} */({}), {
                get: (t,p,r) => {
                    ...
                    if(this._where) {
                        // @ts-ignore This is private, but this is an exception so Views can work appropriately.
                        this._where._current = { property: `${table}.${String(p)}`, chain: "AND NOT" };
                        return this._where;
                    }
                    this._where = Where(String(p), table, this._relationships, "WHERE NOT");
                    return this._where;
                }
            });
            whereCallback(newProxy(this._realTableName));
     ...
     * 
     * __NOTE: This also works on keys from included tables, including one-to-many relationships, just reference the included model further until you reach the property desired.__
     * @param {Types.SortByCallback<TTableModel>} orderByCallback Callback to help present the keys you'd like to sort on.
     ...
     * 
     * __NOTE: This also works on keys from included tables, including one-to-many relationships, just reference the included model further until you reach the property desired.__
     * @param {(model: Types.GroupByCallbackModel<Types.AllKeysRequired<TTableModel>>, aggregates: Types.Aggregates<TTableModel>) => string|string[]} groupByCallback
    ...
    groupBy(groupByCallback) {
        this._promises = [Promise.all(this._promises).then(() => {
            ...
            /** @type {Types.Aggregates<TAliasMap>} */
            const aggregates = {
        ...
        /** @type {MyORMContext<TTableModel, TAliasMap>} */
        const ctx = new MyORMContext(this._adapter, this._realTableName);
     ...
     * @template {Types.AbstractModel} TAliasedType Aliased type that is derived from the return value of `aliasModelCallback`.
    ...
    alias(aliasModelCallback) {
		...
		/** @type {MyORMContext<TTableModel, Types.NonPrimitiveTypesAsOptional<Types.UndefinedAsOptional<TAliasedType>>>} */
		const ctx = new MyORMContext(this._adapter, this._realTableName, this._options, this._schema);
		this._transferToNewContext(/** @type {any} */ (ctx));
		return ctx;
	}
    ...
    async count() {
        ...
        return await this._adapter.handleCount(cmd, args);
    }
    ...
    async insert(records) {
        ...
        if(this._aliasCallback) {
            return recordsMappedBack.map((r) => {
				...
				let aliased = this._aliasCallback(newProxy());
    ...
    async updateSelect(propertiesToUpdate) {
        ...
        await Promise.all(this._promises);
        if ((this._where == null || this._where.getArgs().length <= 0) && !this._options.allowUpdateOnAll) {
    ...
    async delete(records) {
        ...
        if(!primaryKey) {
            throw new MyORMSyntaxError(`No primary key was identified in any of the records passed in.`);
        }
     ...
     */
    onSuccess(callback) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, callback);
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, callback);
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, callback);
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, callback);
    }
     ...
     */
    onFail(callback) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_QUERY_FAILED}-${this._realTableName}`, callback);
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_INSERT_FAILED}-${this._realTableName}`, callback);
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_UPDATE_FAILED}-${this._realTableName}`, callback);
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_DELETE_FAILED}-${this._realTableName}`, callback);
    }
    ...
    onQuerySuccess(success) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, success);
    }
    ...
    onInsertSuccess(success) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, success);
    }
    ...
    onUpdateSuccess(success) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, success);
    }
    ...
    onDeleteSuccess(success) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, success);
    }
     ...
     */
    onQueryFail(fail) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, fail);
    }
     ...
     */
    onInsertFail(fail) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, fail);
    }
     ...
     */
    onUpdateFail(fail) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, fail);
    }
     ...
     */
    onDeleteFail(fail) {
        this._emitter.addListener(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, fail);
    }

    /**
     * Transfers details from this context to another context.
     * @template {Types.AbstractModel} TAliasdType
     * @private
     ...
     * @private
     * @param {Types.HasOneCallback<TTableModel>|Types.HasManyCallback<TTableModel>} relationshipCallback Used to configure the keys for the informal foreign relationship.
    ...
    /**
     * Returns all of the data built using clause functions.
     * @private
     * @param {string} table Name of the table to filter the where clauses on. (default: "",  or all conditions)
     */
    _getClauseData(table="") {
        ...
        let where = this._where?.toString(table);
        let groupBy = this._groupBy?.map(col => 
            col.includes(" AS ") // if it includes "AS" then it was an aggregate function, so then this won't be added to the group by clause.
                ? "" 
                : `${col}`
            ).filter(s => s != "");
        let orderBy = this._sortByKeys;
        let limit = this._limit;
        let offset = this._offset;
        let includes = Object.values(this._relationships)
            .filter(m => m.included)
        ...
        return {
            ...
            limit,
            offset,
            whereArgs: this._where?.getArgs(table) ?? []
        }
    }

    /**
     * Gets all of the clauses in their stringified form.
     * @private
     */
    _getClauses() {
        let selects = [
        ...
        const from = "\n\tFROM " + [thisTable, ...Object.values(this._relationships)
    ...
    /**
     * Serializes the list of `records` into their appropriate end-user expected structure.
     * @private
    ...
    _serialize(thisTable, records) {
        ...
        if (records.length > 0) {
            ...
            if (primaryKey && Object.values(this._relationships).filter(v => v.included).length > 0) {
                ...
                if (Object.values(this._relationships).filter(v => v.included && v.thisTable == thisTable && v.type == "1:n").length > 0) {
                    recs = records.filter((t, n, self) => n === self.findIndex(_t => _t[primaryKey] === t[primaryKey]));
                }
                records = recs.map(t => this._mapResults(t, records, /** @type {{[K in keyof TAliasMap]: string|object|((record: any, allRecords: any[]) => any[])}} */(mappingObject)));
            }
        }
        return records;
    }
     ...
     * @private
     * @param {TAliasMap[]|TTableModel[]} records Records being aliased.
     * @returns {TAliasMap[]} Same records returned in their aliased state.
     */
    _alias(records) {
    ...
    async _query(cmd, args) {
        try {
            ...
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, this._createEventDetail(cmd, args));
            return /** @type {any} */ (result);
        } catch (err) {
            const detail = this._createEventDetail(cmd, args, err);
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_QUERY_FAILED}-${this._realTableName}`, detail);
    ...
    async _insert(cmd, args = undefined) {
        try {
            const [result] = /** @type {import('mysql2/promise').ResultSetHeader[]} */ (await this._adapter.connection.execute(cmd, args));
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, this._createEventDetail(cmd, args));
            return Array.from(Array(result.affectedRows).keys()).map((_, n) => n + result.insertId);
        } catch (err) {
            const detail = this._createEventDetail(cmd, args, err);
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_INSERT_FAILED}-${this._realTableName}`, detail);
    ...
    async _update(cmd, args = undefined) {
        try {
            const result = /** @type {import('mysql2/promise').ResultSetHeader} */ ((await this._adapter.connection.execute(cmd, args))[0]);
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, this._createEventDetail(cmd, args));
            return result.affectedRows;
        } catch (err) {
            const detail = this._createEventDetail(cmd, args, err);
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_UPDATE_FAILED}-${this._realTableName}`, detail);
    ...
    async _delete(cmd, args = undefined) {
        try {
            const result = /** @type {import('mysql2/promise').ResultSetHeader} */ ((await this._adapter.connection.execute(cmd, args))[0]);
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, this._createEventDetail(cmd, args));
            return result.affectedRows;
        } catch (err) {
            const detail = this._createEventDetail(cmd, args, err);
            this._emitter.emit(`${EVENT_TABLE_CONTEXT_DELETE_FAILED}-${this._realTableName}`, detail)
    ...
    async _describe(table, then, optionalPrependAlias="") {
        ...
        const schema = {};
        const [cols] = await this._adapter.connection.query(`DESCRIBE ${table}`);
    ...
    /**
     * Gets the primary key of the table. If no primary key exists, then `undefined` is returned.
     * @private
     * @returns {string=}
     */
    _getPrimaryKey() {
    ...
    /**
     * Returns true if the key specified is has the `AUTO_INCREMENT` attribute on it.
     * @private
     * @param {string=} key 
     * @returns {boolean} True if the key is an auto increment key. False if the key does not exist in the schema or does not have an `AUTO_INCREMENT` attribute.
     */
    _isIdentityKey(key) {
    ...
    /**
     * Creates a detail to be passed into an emitted event.
     * @private
     * @param {string} cmd Command passed into the adapter
     * @param {any[]=} args Respective arguments to the `cmd`.
     * @param {Error=} err Error that was thrown.
     * @param {number=} numRowsAffected Number of rows affected
     * @returns {any} `OnSuccessData` or `OnFailData` based on if `err` is defined as some JS Error.
     */
    ...
    /**
     * Returns the tools used for the `.onSerialization()` function on the adapter.
     * @private
     * @returns {Types.OnSerializationTools<TTableModel>}
     */
    _createAdapterToolsDetail() {
        return { MyORMError: MyORMSyntaxError, Where, Schema: this._schema, Relationships: this._relationships };
    }

    /**
     * Returns info on the state of the command being executed.
     * @private
     * @param {boolean} isExplicit True if the command being executed is an explicit transaction (uses built clauses), otherwise false. (default: true)
     * @param {boolean} isCount True if the command being executed is "count", otherwise false. (default: false)
    ...
    _createAdapterContextDetail(isExplicit = true, isCount = false) {
        ...
        const cmdCtx = {
            ...
            isCount,
            isExplicit, 
            hasOneToOne: Object.values(this._relationships).map(v => v.included && v.type == "1:1").length > 0,
            hasOneToMany: Object.values(this._relationships).map(v => v.included && v.type == "1:n").length > 0
        };
        return cmdCtx;
    }

    /**
     * Augment all strings that appear like JavaScript dates to be in MySQL
     * @private
     * @param {any[]} args 
     * @returns {any[]}
     */
