...
import { ALIAS_AGGREGATE_SEPARATOR, ALIAS_TABLE_SEPARATOR } from './util.js';
...
export class MyORMContext {
     ...
     * @param {Types.MyORMAdapter<TTableModel>} adapter 
     * Adapter that handles serialization and command execution for built commands. 
     * @param {string} realTableName 
     * Name of the table in your database this context is connecting to.
     * @param {Types.TableContextOptions} options 
     * Context options that enable certain features, such as truncation, updating all, or sorting query result keys.
     * @param {{ [key: string]: { Field: string; Type: string; Null: string; Key: string; Default: string; Extra: string; Alias: string; } }=} schema 
     * Schema as described when first initializing this object. This is only meant for internal use.
     * @param {CommandListener=} emitter 
     * An existing emitter
     */
     ...
     * @returns {MyORMContext<TTableModel, TAliasMap & {[K in keyof TAugmentedType]?: K extends keyof TTableModel ? TTableModel[K] extends Types.AbstractModel[]|undefined ? TTableModel[K] : TTableModel[K] : never}>} 
    ...
    sortBy(orderByCallback) {
        return this.#transfer(ctx => {
            ...
            ctx.#state.sortBy = Array.isArray(sbKeys) ? sbKeys : [sbKeys];
        });
    }
    ...
    #getClauseData(table="") {
        ...
        let includes = Object.values(this.#state.relationships)
            ...
            .filter(m => m.included)
            .map(ic => `\`${ic.thatTable}\` AS \`${ic.alias}\` ON \`${ic.thisTable}\`.\`${ic.primaryKey}\`=\`${ic.alias}\`.\`${ic.foreignKey}\``)
            ...
            for(const key in record) {
                ...
                if(table === key) {
                    ...
                    if (this.#state.relationships[table].type === "1:1" || this.#state.groupBy) {
                        const _r = map(r);
                        mapping[table] = Object.keys(r).length <= 0 ? null : _r;
                    } else {
    ...
    async #describe(table, optionalPrependAlias="", alias=table) {
        ...
        for(const field of schemaFields) {
            schema[field.Field] = { ...field, Field: `\`${alias}\`.\`${field.Field}\``, Alias: `\`${describedTable}${field.Field}\`` };
        }
        return schema;
    }
