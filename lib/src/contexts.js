...
import { 
    ...
    MyORMOptionsError, 
    MyORMQueryError, 
    MyORMRelationshipError, 
    MyORMSyntaxError, 
    MyORMUpdateError, 
    RelationshipErrorTypes } from './exceptions.js';
import { Where } from './where-builder.js';
import { CommandListener } from './events.js';
import * as Types from './types.js';
import { ALIAS_AGGREGATE_SEPARATOR, ALIAS_TABLE_SEPARATOR, DEFAULT_ESCAPE_CHARACTER__START, DEFAULT_ESCAPE_CHARACTER__END } from './util.js';
...
export class MyORMContext {
    ...
    constructor(adapter, 
        realTableName, 
        options=undefined,
        schema = undefined, emitter = undefined) 
    {
        ...
        this.#options = { 
            ...
            sortKeys: false,
            escapeCharStart: adapter.options.escapeCharStart ?? DEFAULT_ESCAPE_CHARACTER__START,
            escapeCharEnd: adapter.options.escapeCharEnd ?? DEFAULT_ESCAPE_CHARACTER__END,
            ...options,
        };
    ...
    /**
     * Preset a callback for automatically assigning a value to your primary keys, this is called on every record that is inserted using the `.insert()` function.  
     * 
     * This function is intended for tables that do not have an `AUTO_INCREMENT` attribute on their primary key.  
     * 
     * @param {(m: Partial<TTableModel>) => Types.MaybePromise<TTableModel[keyof Types.OnlyNonAbstractModels<TTableModel>]>} identityCallback
     * Used to assign some value to the primary key determined on a record (or records) being inserted.
     */
    identify(identityCallback) {
        if(!this.#isIdentityKey(this.#getPrimaryKey())) {
            this.#state.identityCallback = identityCallback;
        }
        return this;
    }
            ...
            const newProxy = () => new Proxy(/** @type {any} */({}), {
                get: (t, p, r) => {
                    if (p in ctx.#state.relationships) {
                        ...
                        throw new MyORMRelationshipError(RelationshipErrorTypes.NOT_CONFIGURED, String(p));
                    }
                    return {
    ...
    where(whereCallback) {
        return this.#transfer(ctx => {
            const newProxy = (table) => new Proxy(/** @type {any} */({}), {
                get: (t, p, r) => {
                    if (p in ctx.#state.relationships) {
                        return newProxy(ctx.#state.relationships[p].alias);
                    }
                    if (ctx.#state.where) {
                    ...
                    ctx.#state.where = Where(String(p), table, ctx.#state.relationships, ctx.#adapter.options, "WHERE");
                    return ctx.#state.where;
                }
            });
        ...
        return this.#transfer(ctx => {
            const newProxy = (table) => new Proxy(/** @type {any} */({}), {
                get: (t, p, r) => {
                    ...
                    this.#state.where = Where(String(p), table, ctx.#state.relationships, ctx.#adapter.options, "WHERE NOT");
                    return this.#state.where;
                }
            });
            whereCallback(newProxy(this.#realTableName));
        ...
        return this.#transfer(ctx => {
            ...
            const newProxy = (table = ctx.#realTableName) => new Proxy(/** @type {any} */({}), {
                get: (t, p) => {
                    ...
                    const sbk = {
                        column: String(p).startsWith("$") ? this.#escape(String(p)) : `${table}.${this.#escape(String(p))}`,
                        direction: "ASC",
                        asc: () => {
                            sbk.direction = "ASC";
                            return sbk;
    ...
    groupBy(groupByCallback) {
        return this.#transfer((ctx) => {
            ...
            const newProxy = (table = `${this.#escape(ctx.#realTableName)}.`, fullAlias = "") => new Proxy(/** @type {any} */({}), {
                get: (t, p, r) => {
                    if (p in ctx.#state.relationships) {
                        if (ctx.#state.relationships[p].included) {
                            return newProxy(`${ctx.#state.relationships[p].alias}.`, `${fullAlias}${String(p)}${ALIAS_AGGREGATE_SEPARATOR}`);
                        } else {
                            throw new MyORMRelationshipError(RelationshipErrorTypes.CONFIGURED_BUT_NOT_INCLUDED, String(p));
                        }
                    }
            ...
            function removeSubProps(col) {
                const [table, column] = col
                    .replace(new RegExp(`\\${self.#options.escapeCharStart}`, "g"), "")
                    .replace(new RegExp(`\\${self.#options.escapeCharEnd}`, "g"), "")
                    .split(".");
                const columns = column.split(ALIAS_AGGREGATE_SEPARATOR);
                return `${table}.${self.#escape(columns[columns.length - 1])}`;
            }
            ...
            const groups = groupByCallback(newProxy(), aggrs);
    ...
    async insert(records) {
        ...
        if(this.#state.identityCallback) {
            recordsMappedBack = recordsMappedBack.map(r => {
                if(!this.#state.identityCallback || !primaryKey) return r;
                //@ts-ignore problems because recordsMappedBack is typed as TAliasMap | TTableModel
                r[primaryKey] = this.#state.identityCallback(/** @type {TTableModel} */(r));
                return r;
            });
        }
        ...
        const cols = keysFiltered.map(k => `${this.#realTableName}.${this.#escape(k)}`);
        ...
        if(this.#state.mapForward) {
            ...
            return recordsMappedBack.map((r) => {
				...
				let aliased = this.#state.mapForward(newProxy(r));
    ...
    async updateSelect(propertiesToUpdate) {
        ...
        if ((this.#state.where == null || this.#state.where.getArgs().length <= 0) && !this.#options.allowUpdateOnAll) {
            if ('allowTruncation' in this.#adapter.options && !this.#adapter.options.allowTruncation) throw new MyORMAdapterError('UPDATE_ALL_DISABLED');
            if (!this.#options.allowUpdateOnAll) throw new MyORMOptionsError(`The command specified possibly results in an update to all of your records in the database. If this was intended, pass true to the 'allowUpdateOnAll' property in the options object in the constructor.`);
            this.#warn(`Possible update to all records has been fired`, 'UPDATE');
        }
        ...
        const sets = Object.keys(recordMappedBack)
            ...
            .map((key, n) => {
            return `${this.#escape(key)} = ${this.#adapter.options.useNumberedParameters ? `@arg${n}` : '?'}`;
        });

        console.log({where: this.#state.where?.toString(this.#realTableName, sets.length)});
        ...
        const { cmd, args } = this.#adapter.onSerialization(this.#createAdapterToolsDetail(), this.#createAdapterContextDetail()).forUpdate({ 
            ...
            where: this.#state.where?.toString(this.#realTableName, sets.length),
     ...
     */
    async truncate() {
        if ('allowTruncation' in this.#adapter.options && !this.#adapter.options.allowTruncation) throw new MyORMAdapterError('TRUNCATION_DISABLED');
     ...
     */
    onSuccess(callback) {
        if ("eventHandling" in this.#adapter.options && !this.#adapter.options.eventHandling) throw new MyORMAdapterError('EVENT_HANDLING_DISABLED');
    ...
    /**
     * Adds a listener event to the Connection Pool associated with this context
     * whenever a warning is emitted internally within MyORM (e.g., a truncation has occurred.)
     */
    onWarning(callback) {
        if ("eventHandling" in this.#adapter.options && !this.#adapter.options.eventHandling) throw new MyORMAdapterError('EVENT_HANDLING_DISABLED');
    ...
    #transfer(callback) {
        ...
        ctx.#promise = this.#promise.then(() => {
            ...
            ctx.#state.relationships = JSON.parse(JSON.stringify(this.#state.relationships));
            callback(ctx);
    ...
    #configureRelationship(relationshipCallback, relationshipType, lastTableName = this.#realTableName, fullAliasName="") {
        ...
        const $to = (prop, joiningTableName, originalTableCol, joiningTableCol) => {
            this.#state.relationships[prop] = {
                disposition: Object.keys(this.#state.relationships).length,
                ...
                schema: {},
                alias: `__${fullAliasName.replace(new RegExp("\\<\\|", "g"), "_")}${fullAliasName != "" ? "_" :""}${prop}__`
            };
            this.#promise = this.#promise.then(async () => {
                const schema = await this.#describe(joiningTableName, `${fullAliasName}${fullAliasName != "" ? ALIAS_TABLE_SEPARATOR :""}${prop}`, this.#state.relationships[prop].alias);
                this.#state.relationships[prop].schema = schema;
            });
            return {
                andThatHasOne: (callback) => $andThatHasOne(callback, this.#state.relationships[prop].alias, `${fullAliasName}${fullAliasName != "" ? ALIAS_TABLE_SEPARATOR : ""}${prop}`),
                andThatHasMany: (callback) => $andThatHasMany(callback, this.#state.relationships[prop].alias, `${fullAliasName}${fullAliasName != "" ? ALIAS_TABLE_SEPARATOR : ""}${prop}`)
            };
        }
    ...
    #getClauseData(table="") {
        ...
        if (this.#state.groupBy) {
            selects = [...this.#state.groupBy.map(o => {
                ...
                if (splits.length > 1) {
                    ...
                    const aliases = entries.map(([k,v]) => v.alias);
                    if (splits[0] in this.#state.relationships) {
                        table = splits[0];
                    } else if(aliases.includes(splits[0])) {
                        table = entries.filter(([k,v]) => v.alias == splits[0])[0][0];
                    }
                }
                if (table) {
        ...
        let includes = Object.values(this.#state.relationships)
            // ensure each table is joined correctly by calculating disposition.
            .sort((a,b) => a.disposition - b.disposition)
            .filter(m => m.included)
            .map(ic => `${ic.thatTable} AS ${ic.alias} ON ${ic.thisTable}.${this.#escape(ic.primaryKey)}=${ic.alias}.${this.#escape(ic.foreignKey)}`)
        ...
        return (r) => {
            ...
            for(const key in record) {
                ...
                if(table === key) {
                    ...
                    const map = this.#map(records, Object.fromEntries(entries), `${prepend}${table}${ALIAS_TABLE_SEPARATOR}`);
                    if (this.#state.relationships[table].type === "1:1" || this.#state.groupBy) {
                        r = map(r);
                        ...
                        mapping[table] = this.#filterForUniqueRelatedRecords(records.filter(_r => r[`${prepend}${pKey}`] === _r[`${prepend}${table}${ALIAS_TABLE_SEPARATOR}${fKey}`]), table, `${prepend}${table}${ALIAS_TABLE_SEPARATOR}`).map(map);
                    }
                }
            }
     ...
     */
    #filterForUniqueRelatedRecords(records, table=this.#realTableName, prepend='') {
                ...
                const newProxy = o => new Proxy(o, {
                    get: (target, prop) => {
                        if (prop in this.#state.relationships) {
                            if(this.#state.relationships[prop].type === "1:1") {
    ...
    async #query(cmd, args) {
        try {
            ...
            this.#emitter.emitQuerySuccess(this.#createEventDetail(cmd, args, result));
            return /** @type {any} */ (result);
        } catch (err) {
            const detail = this.#createEventDetail(cmd, args, err);
            this.#emitter.emitQueryFail(detail);
    ...
    async #insert(cmd, args = undefined) {
        try {
            ...
            this.#emitter.emitInsertSuccess(this.#createEventDetail(cmd, args, result));
            return result;
        } catch (err) {
            const detail = this.#createEventDetail(cmd, args, err);
    ...
    async #update(cmd, args = undefined) {
        try {
            ...
            this.#emitter.emitUpdateSuccess(this.#createEventDetail(cmd, args, undefined, undefined, result));
            return result;
        } catch (err) {
            const detail = this.#createEventDetail(cmd, args, err);
    ...
    async #delete(cmd, args = undefined) {
        try {
            ...
            this.#emitter.emitDeleteSuccess(this.#createEventDetail(cmd, args, undefined, undefined, result));
            return result;
        } catch (err) {
            const detail = this.#createEventDetail(cmd, args, err);
     ...
     */
    async #describe(table, optionalPrependAlias="", alias=table) {
        const describedTable = table == this.#realTableName ? "" : `${optionalPrependAlias != "" ? optionalPrependAlias : table}${ALIAS_TABLE_SEPARATOR}`;
        /** @type {{[key: string]: Types.SchemaField}} */
        let schema = {};
        ...
        for(const field of schemaFields) {
            schema[field.Field] = { 
                ...field, 
                Field: `${this.#escape(alias)}.${this.#escape(field.Field)}`, 
                Alias: `${this.#escape(`${describedTable}${field.Field}`)}` 
            };
        }
        return schema;
    }
    ...
    #getPrimaryKey(table=null) {
        if(table == null || table === this.#realTableName) {
            ...
            for (const key in this.#state.relationships[table].schema) {
                if (this.#state.relationships[table].schema[key].Key === "PRI") {
                    return key;
                }
            }
        }
     ...
     * @param {any[]=} resultsInSqlRowFormat
     * Results directly from the adapter from before serialization using .#serialize takes place.
     * @param {Error=} err 
     ...
     */
    #createEventDetail(cmd, args, resultsInSqlRowFormat, err=undefined, numRowsAffected=undefined) {
        let cmdRaw = cmd;
        if (!("eventHandling" in this.#adapter.options) || this.#adapter.options.eventHandling) {
            args?.forEach(a => cmdRaw = cmdRaw.replace(this.#adapter.options.useNumberedParameters ? /@arg[0-9][0-9]*/ : "?", typeof a === "string" || a instanceof Date ? `'${a}'` : a));
        }
    ...
    #createAdapterToolsDetail() {
        return { MyORMError: MyORMAdapterError, Where, Schema: this.#schema, Relationships: this.#state.relationships };
    }
     ...
     */
    #warn(message, type) {
        if ("eventHandling" in this.#adapter.options && !this.#adapter.options.eventHandling) return;
        this.#emitter.emitWarning({ dateIso: new Date().toISOString(), message, type, table: this.#realTableName });
    }

    /**
     * Escape a given string with the strings provided in the keys in options, `escapeCharStart` and `escapeCharEnd`
     * @param {string} s 
     */
    #escape(s) {
        return this.#options.escapeCharStart + s + this.#options.escapeCharEnd;
    }
}
