...
export class MyORMContext {
    ...
    groupBy(groupByCallback) {
        this._promises = [Promise.all(this._promises).then(() => {
            ...
            const newProxy = (table=`\`${this._realTableName}\`.`, fullAlias="") => new Proxy(/** @type {any} */({}), {
                get: (t, p, r) => {
                    if (p in this._relationships) {
                        if (this._relationships[p].included) {
                            return newProxy(`\`${this._relationships[p].thatTable}\`.`, `${fullAlias}${String(p)}_`);
                        } else {
            ...
            const aggrs = this._adapter.onSerialization(this._createAdapterToolsDetail(), this._createAdapterContextDetail()).forAggregates({
            ...
            this._groupBy = this._groupBy.map(col => {
                if(col.includes(" AS ")) return col;
                return removeSubProps(col);
            });
        })];

        /** @type {MyORMContext<Types.ReconstructAbstractModel<TTableModel, TGroupedColumns>, Types.ReconstructAbstractModel<TTableModel, TGroupedColumns>>} */
