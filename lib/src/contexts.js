...
export class MyORMContext {
    ...
    async select() {
        ...
        const thisTable = queryData.from[0];
        const cmdCtx = {
            mainTableName: thisTable,
            isCount: false,
        ...
        const { cmd, args } = this._adapter.onSerialization({ MyORMError: MyORMSyntaxError, Where, Relationships: this._relationships }, cmdCtx).forQuery(queryData);

        /** @type {TAliasMap[]|TTableModel[]} */
        let ts = await this._query(cmd, args);
        ts = this._serialize(thisTable, ts);
        ts = this._alias(ts);
        return ts;
    }
    ...
    async update(records) {
        ...
        for(const record of recordsMappedBack) {
            ...
            if(!primaryKey || !(primaryKey in record)) {
                ...
                throw new MyORMSyntaxError(`Record does not have a primary key specified to update on. (Record: ${JSON.stringify(record)})`);
            } else {
     ...
     */
    onSuccess(callback) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, callback);
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, callback);
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, callback);
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, callback);
    }
     ...
     */
    onFail(callback) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_QUERY_FAILED}-${this._realTableName}`, callback);
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_INSERT_FAILED}-${this._realTableName}`, callback);
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_UPDATE_FAILED}-${this._realTableName}`, callback);
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_DELETE_FAILED}-${this._realTableName}`, callback);
    }
    ...
    onQuerySuccess(success) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, success);
    }
    ...
    onInsertSuccess(success) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, success);
    }
    ...
    onUpdateSuccess(success) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, success);
    }
    ...
    onDeleteSuccess(success) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, success);
    }
     ...
     */
    onQueryFail(fail) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, fail);
    }
     ...
     */
    onInsertFail(fail) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, fail);
    }
     ...
     */
    onUpdateFail(fail) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, fail);
    }
     ...
     */
    onDeleteFail(fail) {
        this._adapter.connection.addListener(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, fail);
    }
    ...
    _getClauseData() {
        ...
        let groupBy = this._groupBy?.map(col => col.includes(" AS ") ? "" : `${col}`).filter(s => s != "");
    ...
    _alias(records) {
        ...
        return /** @type {TAliasMap[]} */ (/** @type {unknown}*/ (records));
    }
    ...
    async _query(cmd, args) {
        ...
        try {
            ...
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_QUERY}-${this._realTableName}`, {
                dateIso: new Date().toISOString(),
                host: (/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.host,
                schema: `[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`,
                cmdRaw,
                cmdSanitized: cmd,
                args
            });
            ...
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_QUERY_FAILED}-${this._realTableName}`, {
                error: err,
                dateIso: new Date().toISOString(),
                host: (/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.host,
                schema: `[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`,
            ...
            throw new MyORMQueryError(`An error occurred when attempting to query from ${`[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
    ...
    async _insert(cmd, args = undefined) {
        ...
        try {
            ...
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_INSERT}-${this._realTableName}`, {
            ...
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_INSERT_FAILED}-${this._realTableName}`, {
                error: err,
                dateIso: new Date().toISOString(),
                host: (/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.host,
                schema: `[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`,
            ...
            throw new MyORMInsertError(`An error occurred when attempting to insert into ${`[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
    ...
    async _update(cmd, args = undefined) {
        ...
        try {
            ...
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_UPDATE}-${this._realTableName}`, {
                affectedRows: result.affectedRows,
                dateIso: new Date().toISOString(),
                host: (/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.host,
                schema: `[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`,
            ...
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_UPDATE_FAILED}-${this._realTableName}`, {
                error: err,
                dateIso: new Date().toISOString(),
                host: (/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.host,
                schema: `[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`,
            ...
            throw new MyORMUpdateError(`An error occurred when attempting to update ${`[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
    ...
    async _delete(cmd, args = undefined) {
        ...
        try {
            ...
            args?.forEach(a => cmdRaw = cmdRaw.replace('?', typeof (a) === "string" || a instanceof Date ? `'${a}'` : a));
            const result = /** @type {import('mysql2/promise').ResultSetHeader} */ ((await this._adapter.connection.execute(cmd, args))[0]);
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_DELETE}-${this._realTableName}`, {
                affectedRows: result.affectedRows,
                dateIso: new Date().toISOString(),
                host: (/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.host,
                schema: `[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`,
            ...
            this._adapter.connection.emit(`${EVENT_TABLE_CONTEXT_DELETE_FAILED}-${this._realTableName}`, {
                error: err,
                dateIso: new Date().toISOString(),
                host: (/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.host,
                schema: `[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`,
            ...
            throw new MyORMDeleteError(`An error occurred when attempting to delete from ${`[${(/** @type {any} */(this._adapter.connection.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
