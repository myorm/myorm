...
export class MyORMContext {
     ...
     * @param {(model: {[K in keyof OnlyAbstractModelTypes<TTableModel>]: ThenIncludeObject<TTableModel[K] extends (infer TType)[] ? TType extends AbstractModel ? TType : never : TTableModel[K]>}) => void} modelCallback Callback where the argument, `model`, only has properties of non-primitive types to provide clarity to what sub-type (or table) should be included (or joined on).
        ...
        const newProxy = () => new Proxy(/** @type {any} */({}), {
            get: (t, p, r) => {
                if(p in this._relationships) {
                    console.log(`Including ${String(p)}`);
    ...
    alias(aliasModelCallback) {
        ...
        console.log(aliases, this._relationships);
        ...
        const recurseForEach = (o, k) => {
            ...
            console.log(reResult[1]);
            if(reResult[1] in this._relationships) {
                console.log("???", k, reResult[1]);
    ...
    async select() {
        let sels = [];
        let thisSchema, includedSchemas;
        // Aliases have been specified.
        if(this._aliases) {
            sels = [
                ...Object.values(this._schema).map(o => `${o.Field} AS ${o.Alias}`),
                ...Object.keys(this._relationships).flatMap(k => Object.values(this._relationships[k].schema).map(o => `${o.Field} AS ${o.Alias}`))
            ];
            console.log(sels);
        } else {
            sels = [
            ...
            for(const key in unwrapped) {
                ...
                if(key in this._relationships) {
                    includedTableSelects = [
                        ...includedTableSelects, 
                        ...Object.entries(unwrapped[key]).map(alias => mapAliasesCallback(this._relationships[key].name, alias))
                    ];
                }
            }
        ...
        if(Object.values(this._relationships).map(v => v.included && v.type == "1:n")) {
            thisTable = `(SELECT * FROM ${thisTable} ${limit} ${offset}) AS ${thisTable}`;
            limit = offset = "";
        }
        const from = [thisTable, ...Object.values(this._relationships).filter(ic => ic.included).map(ic => `\`${ic.name}\` ON \`${this._realTableName}\`.\`${ic.thisKey}\`=\`${ic.name}\`.\`${ic.thatKey}\``)].join('\n\t\tLEFT JOIN ');
        ...
        if(Object.values(this._relationships).map(ic => ic.included).filter(included => included).length > 0) {
            ...
            let oneToManyKeys = Object.keys(this._relationships).filter(k => this._relationships[k].included && this._relationships[k].type == "1:n");
     ...
     */
    async count() {
        const from = [`\`${this._realTableName}\``, ...Object.values(this._relationships).filter(ic => ic.included).map(ic => `\`${ic.name}\` ON \`${this._realTableName}\`.\`${ic.thisKey}\`=\`${ic.name}\`.\`${ic.thatKey}\``)].join('\n\t\tJOIN ');
        const where = this._where != null ? this._where.toString() : "";
    ...
    _transferToNewContext(ctx) {
        // we set the new context's describing to make sure that a race condition doesn't occur 
        //   where a command is attempted to be executed before descriptions have been made.
        ctx._describing = this._describing;
        // we have to transfer everything after all descriptions have been made.
        Promise.all(this._describing).then(() => {
     ...
     * @param {"1:n"|"1:1"} relationshipType
     */
    __configureRelationship(relationshipCallback, relationshipType, lastTableName) {
        const $andThatHasOne = (callback, lastTableName) => {
            return this.__configureRelationship(callback, "1:1", lastTableName);
        };
        const $andThatHasMany = (callback, lastTableName) => {
            return this.__configureRelationship(callback, "1:n", lastTableName);
        };
        const $to = (prop, joiningTableName, joiningTableCol, originalTableCol) => {
            this._relationships[prop] = {
                thisTable: lastTableName,
                thatTable: joiningTableName,
                thisKey: originalTableCol,
                thatKey: joiningTableCol,
                type: relationshipType
            };
            this._describing = [...this._describing, this._describe(joiningTableName, schema => this._relationships[prop].schema = schema)];
            return {
                andThatHasOne: (callback) => $andThatHasOne(callback, joiningTableName),
                andThatHasMany: (callback) => $andThatHasMany(callback, joiningTableName)
            };
        }
        const $with = (prop, realTableName, joiningTableCol) => {
            return {
                to: (originalTableCol) => $to(prop, realTableName, joiningTableCol, originalTableCol)
            };
        }
        const $from = (prop, realTableName) => {
            return {
                with: (thatColumnName) => $with(prop, realTableName, thatColumnName)
            };
        };
        const newProxy = () => new Proxy(/** @type {any} */({}), {
            get: (t, p) => {
                return {
                    ...
                    with: (joiningTableCol) => $with(p, p, joiningTableCol)
                };
            }
        });
     ...
     */
    async _describe(table, then) {
        const describedTable = table == this._realTableName ? "" : `${table}.`;
        ...
        then(schema);
        console.log(`resolved for ${table}: ${then.toString()}`);
    }
}
 ...
 * @typedef {{[K in keyof T as T[K] extends AbstractModel|undefined ? K : never]: Required<T[K]>}} OnlyAbstractModels
 */
 ...
 * @returns {ThenHasCallback<TTo>}
 */

/**
 * @template {AbstractModel|AbstractModel[]} TTableModel
 * @typedef {object} ThenHasCallback
 * @prop {(modelCallback: (model: {[K in keyof OnlyAbstractModels<Required<TTableModel>>]: HasOneCallbackConfig<Required<TTableModel>, K>}) => void) => ThenHasCallback<TTableModel>} andThatHasOne
 * @prop {(modelCallback: (model: {[K in keyof OnlyAbstractModelArrays<Required<TTableModel>>]: HasManyCallbackConfig<Required<TTableModel>, K>}) => void) => ThenHasCallback<TTableModel>} andThatHasMany
 */

/**
 * @template {AbstractModel} TTableModel
 * @callback HasOneCallback
 * @param {{[K in keyof Required<OnlyAbstractModels<TTableModel>>]: HasOneCallbackConfig<TTableModel, K>}} model
 */

/**
 * @template {AbstractModel} TTableModel
 * @callback HasManyCallback
 * @param {{[K in keyof Required<OnlyAbstractModelArrays<TTableModel>>]: HasManyCallbackConfig<TTableModel, K>}} model
 */
 ...
 * @prop {RelationshipFrom<TTableModel, OnlyAbstractModels<TTableModel>[Key]>} from
 * @prop {RelationshipWith<TTableModel, OnlyAbstractModels<TTableModel>[Key]>} with
 */
 ...
 * @prop {RelationshipFrom<TTableModel, OnlyAbstractModelArrays<TTableModel>[Key]>} from
 * @prop {RelationshipWith<TTableModel, OnlyAbstractModelArrays<TTableModel>[Key]>} with
 */

/**
 * @template {AbstractModel} TTableModel
 * @typedef {object} ThenIncludeObject
 * @prop {(modelCallback: (model: {[K in keyof OnlyAbstractModelTypes<TTableModel>]: ThenIncludeObject<TTableModel[K]>}) => void) => ThenIncludeObject<TTableModel>} thenInclude
 */
