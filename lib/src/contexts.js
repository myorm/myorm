...
export class MyORMContext {
    ...
    /** @protected @type {{where?: WhereBuilder<TTableModel>?, aliases?: AbstractModel, limit?: number, offset?: number, sortBy?: { column: keyof TTableModel, direction: "ASC"|"DESC"}[], groupBy?: boolean, includes: Partial<{[K in keyof OnlyAbstractModels<TTableModel>]: { included: boolean, name: string, thisKey: keyof TTableModel, thatKey: string, type: "1:1"|"1:n" }}>}} */ _view = {};
    ...
    groupBy(groupByCallback) {
        ...
        for (const alias in includeAliases) {
            const config = this._includeConfigurations[includeAliases[alias]];
            ...
            delete this._includeConfigurations[includeAliases[alias]];
            this._includeConfigurations[alias] = config;
        }
    ...
    alias(aliasModelCallback) {
        ...
        for(let i = 0; i < includedAliases.length; ++i) {
            ...
            config.included = true;
            delete this._includeConfigurations[ia];
    ...
    async select() {
        ...
        if(this._aliases && Object.values(this._includeConfigurations).map(ic => ic.included).filter(included => included).length > 0) {
    ...
    _transferToNewContext(ctx) {
        ...
        if(this._view) {
            ...
            this._grouped = this._view.groupBy;
