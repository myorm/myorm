...
export class MyORMContext {
    ...
    include(modelCallback) {
        ...
        const ctx = new MyORMContext(this._adapter, this._realTableName, this._options, this._schema, this._emitter);
     ...
     * @param {(model: Types.GroupByCallbackModel<Types.AllKeysRequired<TTableModel>>, aggregates: Types.Aggregates<TTableModel>) => (keyof TGroupedColumns & string)[]|keyof TGroupedColumns & string} groupByCallback
     ...
     * @returns {MyORMContext<{[K in keyof TGroupedColumns]: K extends keyof TTableModel ? TTableModel[K] : number}, {[K in keyof TGroupedColumns]: K extends keyof TTableModel ? TTableModel[K] : number}>} A new context with the all previously configured clauses and the updated groupings.
    ...
    groupBy(groupByCallback) {
        this._promises = [Promise.all(this._promises).then(() => {
            ...
            const aggregates = {
                ...
                count: (col) => /** @type {any} */ (`COUNT(DISTINCT ${String(col)}) AS \`$count_${String(col).split(".")[1].replace(/`/g, "")}\``),
                avg: (col) => /** @type {any} */ (`AVG(${String(col)}) AS \`$avg_${String(col).split(".")[1].replace(/`/g, "")}\``),
                max: (col) => /** @type {any} */ (`MAX(${String(col)}) AS \`$max_${String(col).split(".")[1].replace(/`/g, "")}\``),
                min: (col) => /** @type {any} */ (`MIN(${String(col)}) AS \`$min_${String(col).split(".")[1].replace(/`/g, "")}\``),
                sum: (col) => /** @type {any} */ (`SUM(${String(col)}) AS \`$sum_${String(col).split(".")[1].replace(/`/g, "")}\``)
            }
        ...
        /** @type {MyORMContext<{[K in keyof TGroupedColumns]: K extends keyof TTableModel ? TTableModel[K] : number}, {[K in keyof TGroupedColumns]: K extends keyof TTableModel ? TTableModel[K] : number}>} */
        const ctx = /** @type {any} */ (new MyORMContext(this._adapter, this._realTableName));
            ...
            selects = [...this._groupBy.map(o => {
                ...
                if (table) {
                    if (splits[1] in this._relationships[table].schema) {
                        o = `${this._relationships[table].schema[splits[1]].Field} AS ${this._relationships[table].schema[splits[1]].Alias}`;
                    }
                } else {
     ...
     * @returns {Promise<number>} T models that are returned from the 
     */
    ...
    async _update(cmd, args = undefined) {
        try {
            ...
            this._emitter.emitUpdateSuccess(this._createEventDetail(cmd, args));
...
const EVENT_TABLE_CONTEXT_QUERY = (tableName) => `query-success-${tableName.toLowerCase()}`;
const EVENT_TABLE_CONTEXT_QUERY_FAILED = (tableName) => `query-failed-${tableName.toLowerCase()}`;
const EVENT_TABLE_CONTEXT_INSERT = (tableName) => `insert-success-${tableName.toLowerCase()}`;
const EVENT_TABLE_CONTEXT_INSERT_FAILED = (tableName) => `insert-failed-${tableName.toLowerCase()}`;
const EVENT_TABLE_CONTEXT_UPDATE = (tableName) => `update-success-${tableName.toLowerCase()}`;
const EVENT_TABLE_CONTEXT_UPDATE_FAILED = (tableName) => `update-failed-${tableName.toLowerCase()}`;
const EVENT_TABLE_CONTEXT_DELETE = (tableName) => `delete-success-${tableName.toLowerCase()}`;
const EVENT_TABLE_CONTEXT_DELETE_FAILED = (tableName) => `delete-failed-${tableName.toLowerCase()}`;
