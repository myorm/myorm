...
export class MyORMContext {
     ...
     * 
     * __NOTE: This also works on keys from included tables, including one-to-many relationships, just reference the included model further until you reach the property desired.__
     * @template {import("./typings/contexts.js").AbstractModel} TGroupedType
     * @template {TAliasMap} [TAugmentedType=TAliasMap] Augmented `TTableModel` type to assist with included model aliases.
     * @param {(m: TAugmentedType, aggregate: import("./typings/contexts.js").Aggregates<TAliasMap>) => TGroupedType } groupByCallback
    ...
    async select() {
        ...
        if(ts.length > 0) {
            ...
            if(primaryKey) {
                ...
                ts = resultsFiltered.map(t => this._mapResults(t, ts, /** @type {{[K in keyof TAliasMap]: string|object|((record: any, allRecords: any[]) => any[])}} */ (mappingObject)));
            }
        }
