...
export class MyORMContext {
     ...
     * @template {OnlyAbstractModelTypes<TTableModel>} [TAugmentedType=OnlyAbstractModelTypes<TTableModel>] TTableModel augmented so it only displays Abstract Model types. This will be inferred.
     * @param {(model: {[K in keyof OnlyAbstractModelTypes<Required<TTableModel>>]: ThenIncludeObject<Required<TTableModel>[K] extends (infer TType)[] ? Required<TType> : Required<TTableModel>[K]>}) => void} modelCallback 
     ...
     * @param {(m: import("./where-builder.js").ChainObject<Required<TTableModel>>) => void} whereCallback Builder function to help build a WHERE clause.
     ...
     * 
     * __NOTE: This also works on keys from included tables, including one-to-many relationships, just reference the included model further until you reach the property desired.__
     * @param {SortByCallback<TTableModel>} orderByCallback Callback to help present the keys you'd like to sort on.
    ...
    sortBy(orderByCallback) {
        ...
        const newProxy = (table=`\`${this._realTableName}\`.`) => new Proxy(/** @type {any} */ ({}), {
            get: (t,p) => {
                ...
                /** @type {SortByKeyConfig & DirectionCallbacks} */
                const sbk = {
    ...
    /**
     * @private Private until implemented.  
     * 
     ...
     * @returns {MyORMContext<TTableModel, TGroupedType>} A new context with the all previously configured clauses and the updated groupings.
     */
     ...
     * @template {AbstractModel} [TTableModelRequired=AllKeysRequired<TTableModel>] TTableModel but all keys, root and nested, are required.
     * @param {((model: TTableModelRequired) => TAliasedType)} aliasModelCallback Callback that should return an object that would represent your desired aliased type.
     * @returns {MyORMContext<TTableModel, Partial<TAliasedType> & Pick<TAliasedType, keyof {[K in keyof TAliasedType as undefined extends TAliasedType[K] ? never : K]}>>} 
     ...
     * @private
     * @param {TAliasMap|TAliasMap[]} recordOrRecords 
     */
     ...
     * 
     * __NOTE: If a Primary Key was specified in the constructor, the Insert ID will be updated.__
    ...
    /**
     * Updates the record or records specified in the Table this context represents, based on a built where clause to determine what should be updated.  
     * 
     * If no where clause was built, then record(s) will be updated using their primary key to determine uniqueness.  
     * 
     * If tables were included before this command is executed, then any related data will be subsequently updated.
     * 
     * __NOTE: If no primary key exists on the object, then that record will not be updated.__
     * @param {Partial<TAliasMap>} record Object or objects to insert.
 ...
 * @prop {() => number} count Gets the count of all records from the query.
 * @prop {(modelCallback: (model: {[K in keyof TModel]: string}) => string) => number} avg Gets the average amount across all rows for that field.
 * @prop {(modelCallback: (model: {[K in keyof TModel]: string}) => string) => number} sum Gets the total sum amount across all rows for that field.
 * @prop {(modelCallback: (model: {[K in keyof TModel]: string}) => string) => number} max Gets the maximum amount between all rows for that field.
 * @prop {(modelCallback: (model: {[K in keyof TModel]: string}) => string) => number} min Gets the minimum amount between all rows for that field.
 */
...
/**
 * @template {AbstractModel} TTableModel
 * @typedef {{[K in keyof TTableModel]: TTableModel[K] extends (infer T extends AbstractModel)[]|undefined ? SortByCallbackConfig<Required<T>> : TTableModel[K] extends AbstractModel|undefined ? SortByCallbackConfig<Required<TTableModel[K]>> : SortByKeyConfig & DirectionCallbacks}} SortByCallbackConfig
 */
...
/**
 * @template {AbstractModel} TTableModel
 * @typedef {{[K in keyof TTableModel]: TTableModel[K] extends (infer T extends AbstractModel)[]|undefined ? AllKeysRequired<Required<T[]>> : TTableModel[K] extends AbstractModel|undefined ? AllKeysRequired<Required<TTableModel[K]>> : Required<TTableModel[K]>}} AllKeysRequired
 */
