...
export class MyORMContext {
    ...
    async select() {
        ...
        if (Object.values(this._relationships).map(v => v.included && v.type == "1:n") && this._limit != null) {
        ...
        const from = [thisTable, ...Object.values(this._relationships).filter(ic => ic.included).map(ic => `\`${ic.thatTable}\` ON \`${ic.thisTable}\`.\`${ic.thisKey}\`=\`${ic.thatTable}\`.\`${ic.thatKey}\``)].join('\n\t\tLEFT JOIN ');
        ...
        // Used to recursively serialize our result of a flat object into the correct structure as it was intended.
        const foo = (object, value, allSubKeys, currentKey=allSubKeys[0]) => {
            if(allSubKeys.length <= 0) {
                return object;
            } 
            if(allSubKeys.length <= 1) {
                object[currentKey] = value;
                return object;
            }
            object[currentKey] = currentKey in object ? object[currentKey] : {};
            return foo(object[currentKey], value, allSubKeys.slice(1));
        }
        ...
        for(let i = 0; i < ts.length; ++i) {
            ...
            for(const key in t) {
                ...
                if(subKeys.length > 1) {
                    //@ts-ignore
                    t[subKeys[0]] = {...t[subKeys[0]], ...foo({}, t[key], subKeys.slice(1))};
                    delete t[key];
                }
            }
        }
    ...
    __configureRelationship(relationshipCallback, relationshipType, lastTableName) {
        ...
        const $with = (prop, realTableName, originalTableCol) => {
            return {
                to: (joiningTableCol) => $to(prop, realTableName, originalTableCol, joiningTableCol)
            };
        }
 ...
 * @typedef {Without<T, AbstractModel[]|string|boolean|Date|number|undefined>} OnlyAbstractModels
 */
 ...
 * @typedef {Without<T, AbstractModel|AbstractModel[]|undefined>} OnlyNonAbstractModels
 */
 ...
 * @returns {AndThatHasCallbacks<TTo extends undefined ? never : TTo>} Further `andThatHas_` callbacks to configure nested relationships.
 */
 ...
 * @param {{[K in keyof Required<OnlyAbstractModels<TTableModel>>]: HasOneCallbackConfig<TTableModel, K>}} model Model that has `AbstractModel` types to configure a one-to-one relationship with `TTableModel`
 * @returns {void} 
 */
 ...
 * @param {{[K in keyof Required<OnlyAbstractModelArrays<TTableModel>>]: HasManyCallbackConfig<TTableModel, K>}} model Model that has `AbstractModel[]` types to configure a one-to-many relationship with `TTableModel`
 * @returns {void} 
 */
 ...
 * @prop {RelationshipWith<TTableModel, OnlyAbstractModels<TTableModel>[Key]>} with Configures the key to use in this relationship from `TTableModel`
 */
 ...
 * @prop {RelationshipWith<TTableModel, OnlyAbstractModelArrays<TTableModel>[Key]>} with Configures the key to use in this relationship from `TTableModel`
 */
 ...
 * @prop {(modelCallback: (model: {[K in keyof OnlyAbstractModelTypes<TTableModel>]: ThenIncludeObject<TTableModel[K]>}) => void) => ThenIncludeObject<TTableModel>} thenInclude Callback to execute when including a nested table from another inclusion into the context.
 */
...
// thenInclude types. (@TODO needs documentation)
