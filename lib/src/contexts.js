    ...
    #configureRelationship(relationshipCallback, relationshipType, lastTableName = this.#realTableName, fullAliasName="") {
        ...
        const $to = (prop, joiningTableName, originalTableCol, joiningTableCol) => {
            ...
            this.#promise = this.#promise.then(async () => {
                const schema = await this.#describe(joiningTableName, `${fullAliasName}${fullAliasName != "" ? ALIAS_TABLE_SEPARATOR :""}${prop}`);
                this.#state.relationships[prop].schema = schema;
            });
            return {
                andThatHasOne: (callback) => $andThatHasOne(callback, joiningTableName, `${fullAliasName}${fullAliasName != "" ? ALIAS_TABLE_SEPARATOR : ""}${prop}`),
                andThatHasMany: (callback) => $andThatHasMany(callback, joiningTableName, `${fullAliasName}${fullAliasName != "" ? ALIAS_TABLE_SEPARATOR : ""}${prop}`)
            };
        }
