...
export class MyORMContext {
     ...
     * @template {Types.AllKeysRequired<TTableModel>} [TRequiredModel=Types.AllKeysRequired<TTableModel>]
     ...
     * @returns {MyORMContext<TTableModel, Types.UndefinedAsOptional<TAliasedType>>} 
    ...
    alias(aliasModelCallback) {
        this._promises = [Promise.all(this._promises).then(() => {
            ...
            const flipd = Object.fromEntries(Object.entries(aliasMap).filter(([k,v]) => typeof(v) !== "object").map(([k, v]) => [v, k]));
            this._mapBack = (x) => {
        ...
        /** @type {MyORMContext<TTableModel, Types.UndefinedAsOptional<TAliasedType>>} */
        const ctx = new MyORMContext(this._pool, this._realTableName, this._options, this._schema);
        this._transferToNewContext(/** @type {any} */(ctx));
        return ctx;
    }
    ...
    async select() {
        ...
        const { thisTable, selects, from, where, orderBy, groupBy, limit, offset, args } = this._getClauses();
        const cmd = `SELECT ${selects}`
            + ` ${from}`
            + ` ${where}`
            + ` ${orderBy}`
            + ` ${groupBy}`
    ...
    async count() {
        await Promise.all(this._promises);
        const { from, where, orderBy, groupBy, limit, offset, args } =
			this._getClauses();
		const cmd =
    ...
    async _query(cmd, args = undefined) {
        ...
        try {
            ...
            throw new MyORMQueryError(`An error occurred when attempting to query from ${`[${(/** @type {any} */(this._pool.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
    ...
    async _insert(cmd, args = undefined) {
        ...
        try {
            ...
            throw new MyORMInsertError(`An error occurred when attempting to insert into ${`[${(/** @type {any} */(this._pool.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
    ...
    async _update(cmd, args = undefined) {
        ...
        try {
            ...
            throw new MyORMUpdateError(`An error occurred when attempting to update ${`[${(/** @type {any} */(this._pool.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
    ...
    async _delete(cmd, args = undefined) {
        ...
        try {
            ...
            throw new MyORMDeleteError(`An error occurred when attempting to delete from ${`[${(/** @type {any} */(this._pool.pool.config)).connectionConfig.database}].[dbo].[${this._realTableName}]`}.`, err);
        }
    }
