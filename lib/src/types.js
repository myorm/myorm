 ...
 * @prop {((t: U) => number|bigint|string)=} identityCallback Function used to auto-assign values to primary keys. (if the primary key has an `auto_increment` attribute, then this will never exist)
 */
 ...
 * @template {string|symbol|number} K
 * @typedef {K extends `${infer A}${typeof import('./util.js').ALIAS_AGGREGATE_SEPARATOR}${infer B}` ? A : K} Car
 */

/**
 * Grabs the remaining elements in the String, separated by "_".
 * @template {string|symbol|number} K
 * @typedef {K extends `${infer B}${typeof import('./util.js').ALIAS_AGGREGATE_SEPARATOR}${infer A}` ? A : never} Cdr
 */
