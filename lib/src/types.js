 ...
 * @prop {AbstractModel} values Used in an `explicit transaction`.  
 ...
 * Handles execution of a truncate command, given the command string and respective arguments for the comamnd string.  
 * This should return a number representing the total number of rows affected from the command.
 ...
 * @typedef {{[K in keyof Required<T> as T[K] extends (AbstractModel[]|undefined) ? never : T[K] extends AbstractModel|undefined ? Date extends T[K] ? never : K : never]-?: T[K] extends (AbstractModel[]|undefined) ? never : T[K] extends AbstractModel|undefined ? Exclude<T[K], undefined> : never}} OnlyAbstractModels
 */
 ...
 * @template {AbstractModel} T 
 * The abstract model to check properties for recursive `AbstractModel`s.
 * @typedef {{[K in keyof T as T[K] extends AbstractModel[]|AbstractModel|undefined ? Date extends T[K] ? K : never : K]: T[K]}} OnlyNonAbstractModels
 */
